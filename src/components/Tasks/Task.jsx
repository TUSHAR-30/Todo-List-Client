...
function Task() {
    ...
    return (
        <>
            {tasks.map((task) => {
                return (
                    <div key={task.id} className='task' onClick={(e) => { openModal(e, task, "view") }}>
                        ...
                        <div className='task-editicon' onClick={(e) => openModal(e, task, "edit")}>
                            <GrEdit size={17} />
                        </div>
                    </div>
                )
            ...
            <Modal show={isModalOpen} onClose={() => setIsModalOpen(false)}>
                {
                    openedTaskOperation == "edit" ? (
                        <EditTaskModalContent openedTask={openedTask} setOpenedTask={setOpenedTask} setIsModalOpen={setIsModalOpen} />
                    ) :
                        (
                            <ViewTaskModalContent openedTask={openedTask} setIsModalOpen={setIsModalOpen} />
                        )
                }
